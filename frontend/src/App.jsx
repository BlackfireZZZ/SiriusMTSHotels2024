...
function App() {
  const [description, setDescription] = useState("**Отель Пример отеля: Ваш новый оазис в сердце Москвы**\n" +
      "\n" +
      "**Описание:**\n" +
      "Отель \"Пример отеля\" — это 100 просторных и стильных номеров, расположенных в самом сердце Москвы, всего в нескольких шагах от Московского Кремля, Большого Театра и других незабываемых достопримечательностей. Этот современный отель предлагает гостям не только комфорт и уют, но и возможность насладиться всеми благами современного города.\n" +
      "\n" +
      "**Насладитесь:**\n" +
      "- **Девушки в холле** — очаровательные сотрудники, всегда готовые помочь и предложить совет.\n" +
      "- **Хорошая столовая** — изысканные блюда, подающие к завтраку, обеду и ужину, создадут незабываемые моменты.\n" +
      "- **Красивые девушки** — уютные номера с панорамными видами на Кремль, идеальные для романтического отдыха.\n" +
      "- **Бассейн** — освежающий отдых после напряженного дня.\n" +
      "- **Бесплатный Wi-Fi** — без ограничений, для всех гостей, поддерживающий продуктивность и связь с миром.\n" +
      "\n" +
      "**Неподалеку:**\n" +
      "- **Московский Кремль** и **Большой театр** — всего в шаге, чтобы каждый день был наполнен яркими впечатлениями.\n" +
      "- **Храм Василия Блаженного** и **Государственный исторический музей** — для тех, кто ценит историю и культуру.\n" +
      "- **Музей С.С. Прокофьева** и **Русский национальный музей** — погружение в мир искусства и культуры.\n" +
      "\n" +
      "**Отель \"Пример отеля\"** — не просто место для отдыха, а ключ к открытию новых горизонтов, где каждый уголок наполнен историей, культурой и неповторимым духом Москвы.\n");
  ...
  useEffect(() => {
    ...
    if (!sessionCookie) {
      ...
      Cookies.set("session", newSession, {expires: 7}); // срок действия 7 дней
      Cookies.set("stage", 1, {expires: 7});
      console.log("Новая сессия создана:", newSession);
    } else {
      console.log("Существующая сессия:", sessionCookie);
    }
    if (!conversationCreated) {
      // Отправляем POST-запрос на {base_url}/conversation/create
      axios.post(`${base_url}/conversation/create`, {
        session_id: sessionId,
      })
          .then((response) => {
            console.log("Сессия создана на сервере:", response.data);
          })
          .catch((error) => {
            console.error("Ошибка при создании сессии на сервере:", error);
          });
    }
  }, []);
                    ...
                    <div
                      ...
                      <div className="vc_column-inner  ">
                        <div className="wpb_wrapper">
                          ...
                          <div className="ld-fancy-heading ld_fancy_heading_634d4b2e05610" style={{width: '340px'}}>
                            <h1 className="ld-fh-element lqd-highlight-custom-underline lqd-highlight-grow-left text-decoration-default is-in-view"
                              ...
                              <mark className="lqd-highlight"><span className="lqd-highlight-txt">Прокачаем </span><span
                                  className="lqd-highlight-inner" style={{transitionDelay: '0ms'}}><svg
                                  xmlns="http://www.w3.org/2000/svg" width="200.509" height="13.504"
                          ...
                          <div className="ld-fancy-heading ld_fancy_heading_634d4b2e05cbd"><p
                            ...
                            Поможем гостиничному бизнесу развиваться, автоматизировать работу и увеличивать доход.</p></div>
                          <div
